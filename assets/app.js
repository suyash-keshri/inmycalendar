...
function exportCsv(){
  var rows = [["date","status","priority","task","entered_todo","entered_in_progress","entered_done"]];
  ...
  for (var i=0;i<s.length;i++)
    rows.push([s[i].date, s[i].status, s[i].order+1, s[i].text, s[i].ts.todo, s[i].ts.doing, s[i].ts.done]);
